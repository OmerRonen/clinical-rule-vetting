...
class Dataset(DatasetTemplate):
    # rulevetting.DATA_PATH
    def clean_data(self, data_path: str = "data", **kwargs) -> pd.DataFrame:
        raw_data_path = "data/iai_pecarn/raw/"
    ...
    def extract_features(self, preprocessed_data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        df = df[feats]
        df = df.loc[df['GCSScore'] >= 14, :]
        df = df.drop(
            columns=['InitHeartRate', 'InitSysBPRange', 'OtherMOIHighRisk', 'AbdTrauma', 'AbdomenPain', 'SeatBeltSign'
                , 'AbdTenderDegree', 'AbdDistention', 'Race', 'Age', 'GCSScore'], axis=1)
        ...
        categoric_columns = {'LtCostalTender', 'RtCostalTender', 'MOI'}
        numeric_cols = list(all_cols.difference(categoric_columns))

        df = one_hot_encode_df(df, numeric_cols=numeric_cols)
        # print(df.isna().sum())
...
#             # impute missing values
#             # fill in values for some vars from unknown -> None
#             df.loc[df['AbdomenTender'].isin(['no', 'unknown']), 'AbdTenderDegree'] = np.nan
#
#             # # pandas impute missing values with median
#             # df = df.fillna(df.median())
#             # df.GCSScore = df.GCSScore.fillna(df.GCSScore.median())
...
#
#         # convert feats to dummy
#         df = pd.get_dummies(df, dummy_na=False)  # treat na as a separate category
#
#         # remove any col that is all 0s
#         df = df.loc[:, (df != 0).any(axis=0)]
#
